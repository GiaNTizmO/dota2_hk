...
namespace draw
{
	void draw_filled_rect(float x, float y, float w, float h, int a, int r, int g, int b)
	{
		x += leftR;
		y += topR;
		w += leftR;
	...
	void draw_border_box(int x, int y, int w, int h, int thickness, int a, int r, int g, int b)
	{
		draw_filled_rect(x, y, x + w, y + thickness, a, r, g, b);
		draw_filled_rect(x, y, x + thickness, y + h, a, r, g, b);
		draw_filled_rect(x, y + h, x + w, y + h + thickness, a, r, g, b);
		draw_filled_rect(x + w, y, x + w + thickness, y + h + thickness, a, r, g, b);
	}

	void draw_border_box(int x, int y, int w, int h, int thickness, D3DCOLOR color)
	{
		draw_filled_rect(x, y, x + w, y + thickness, color);
		draw_filled_rect(x, y, x + thickness, y + h, color);
		draw_filled_rect(x, y + h, x + w, y + h + thickness, color);
		draw_filled_rect(x + w, y, x + w + thickness, y + h + thickness, color);
	}
	...
	void draw_string(char* String, int x, int y, D3DCOLOR color, ID3DXFont* font)
	{
		x += leftR;
		y += topR;
		RECT FontPos;
	...
	void draw_string(char* String, int x, int y, int len, D3DCOLOR color, ID3DXFont* font)
	{
		x += leftR;
		y += topR;
		RECT FontPos;
	...
	void draw_stringW(wchar_t* String, int x, int y, int len, D3DCOLOR color, ID3DXFont* font)
	{
		x += leftR;
		y += topR;
		RECT FontPos;
	...
	void draw_circle(float x, float y, float rad, float rotate, int resolution, DWORD color)
	{
		x += leftR;
		y += topR;
...
public:
	Resource(int resource_id, const string& resource_class) {
...
namespace DirectX
{
	void Render()
	{
		...
		ID3DXFont* pFont;
		D3DXCreateFont(p_Device, 12, 0, FW_NORMAL, 1, false, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE, "Verdana", &pFont);

		draw::draw_string((char*)("github.com/carterrrj/dota2_hk"), 10, 10, 255, 255, 255, 255, pFont);
	}
}
